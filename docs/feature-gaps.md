...
2. **A searchable raw-telemetry table** — every measurement the vendor returned for the newest sample. The Worker builds it by allowing fields rather than stripping them, so an id under a name nobody has seen yet is still left out.
