...
read the dashboard, and from 3.0 only the owner can.
...
- The dashboard's test data is generated by running the real Worker against the
  vendor fixtures, so a field the Worker stops sending also disappears from the
  tests, and they fail.
...
- **Ask SolarLens**: a question box in the app. "Which month produced the most
  this year?", "How much did I export last week?", "Compare both systems per
...
- **1 first**: its generated test data backs every later phase.
